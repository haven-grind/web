                    ...
                    <div className="block md:flex md:space-x-10">
                        {game.thumbnail ? (
                            <div className="relative mb-6 aspect-video w-full overflow-hidden rounded-lg">
                                <img src={game.thumbnail} alt={game.title} className="h-full w-full border object-contain" />
                            </div>
                        ) : (
                                    ...
                                    game.screenshots.map((screenshot, i) => (
                                        <div key={i} className="relative aspect-video overflow-hidden rounded-md">
                                            <img
                                                src={screenshot}
                                                alt={`Screenshot ${i + 1}`}
                                                className="h-full w-full object-contain transition-transform duration-200 ease-in hover:scale-110"
                                            />
                                        </div>
                                    ))
                                ) : (
                                    <div className="col-span-4 text-center text-gray-500 dark:text-gray-400">No Screenshots Available</div>
                                )}
                            </div>
                        </div>
                    </div>
